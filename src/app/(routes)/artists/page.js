    ...
    <main className="flex flex-row justify-center items-center h-screen">
      <div className="flex flex-col items-center">
        ...
        <div className="flex flex-row justify-center items-center gap-1">
          <Link className="text-3xl text-[#D9D9D9] opacity-50 cursor-pointer" href={ `/mood?mood=${mood}` }>•</Link>
        ...
        <div className="mt-20 grid gap-10 grid-cols-5">
          { ARTISTS.map((artist, index) => 
              <div key={index} className="flex flex-col items-center gap-2 cursor-pointer"
                  onClick={ () => {
                    if(artists.includes(index)) return setArtists(artists.filter(a => a !== index))
                    setArtists([...artists, index]);
                  }}>
                <svg viewBox="0 0 512 512" className={ artists.includes(index) ? "fill-green-400" : "fill-white"}>
                ...
                <h1 className={ `text-xl font-bold ${ artists.includes(index) ? "text-green-400" : "text-white" }` }>{ artist.name }</h1>
              </div>
          )}
        </div>
        ...
        <Link className="mt-20 bg-[#575B71] hover:bg-[#495073] text-center duration-300 text-white font-bold text-4xl p-5 w-1/2 cursor-pointer rounded-lg"
            href={{ pathname: '/generate', query: { mood: mood, artists: artists } }}>
          GENERATE
        </Link>
      </div>
    </main>
